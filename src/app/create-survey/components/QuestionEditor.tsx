...
const QuestionEditor: React.FC<QuestionEditorProps> = ({
    ...
    const handleOptionChange = (event: React.ChangeEvent<HTMLInputElement>, optionIndex: number) => {
    ...
    const handleInputKeyPress = (event: React.KeyboardEvent<HTMLInputElement>, optionIndex: number) => {
        ...
        } else if (event.key === "ArrowDown" && optionIndex + 1 < options.length) {
            event.preventDefault();
        ...
        <div style={{ display: "flex", flexDirection: "column", width: "100%", marginBottom: '2rem', }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: '1rem', marginTop: '1.2rem' }}>
                {options.map((option, optionIndex) => (
                    <div key={optionIndex} style={{ display: "flex", marginLeft: "1rem" }}>
                        <div style={{ height: "1rem", width: "1rem", borderRadius: "100%", border: "1px solid black", opacity: option === "" ? 0.4 : 1 }} />
                        <input
                            ref={(el) => optionRefs.current[optionIndex] = el}
                            style={{ marginLeft: "0.6rem", opacity: option === "" ? 0.4 : 1 }}
                            type="text"
